...
typedef struct { uint8_t kind; uint64_t session; uint32_t request; size_t size; uint8_t data[MUX_MAX_PAYLOAD]; } mux_frame_t;
typedef struct { bool inside, escaped, overflow; size_t size; uint8_t data[MUX_MAX_BODY]; } mux_decoder_t;
